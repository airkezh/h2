...
ddl.moveFallerW = ddl.actor.faller.w;
...
ddl.down = function(downSpeed,countFn,endFn){
	...
	var posBtm = gameBoxHeight - fallerHeight -110;
	moveFaller.css({
		'top':'auto',
		'bottom':posBtm
	});
	...
	downTimmer = setInterval(function(){
		...
		var posBtmPrev = trayHeight*0.5  + (fallerHeight-2)*(count-1) ;
		if(posBtm <=  posBtmPrev ){
			
			moveFaller.css('bottom', posBtmPrev );
			...
			if(judgResult){
				...
				if(ddl.moveFallerW > 15){
					//console.log('> 15 /'+  ddl.moveFallerW);
					if(countFn) {countFn(count)};  //
					ddl.add();
				}else if(ddl.moveFallerW <= 15){
					//console.log('> 15 /'+  ddl.moveFallerW);
					gameOver = true; 
...
ddl.creatFaller = function(randomN){
	...
	$('<div class="faller moveFaller"></div>').css({
		'top':110,
		'left':0,
...
ddl.cutFirst = function(){
	...
	if(moveFaller.offset().left < trayDom.offset().left){
		ddl.moveFallerW =  moveFaller.width() - (trayDom.offset().left - moveFaller.offset().left);
		moveFaller.css({
			'width': ddl.moveFallerW,
			'left': trayDom.offset().left
		});
	}else if(moveFaller.offset().left + moveFaller.width() > trayDom.offset().left + trayDom.width()){
		ddl.moveFallerW =  moveFaller.width() - ( moveFaller.offset().left + moveFaller.width()) - (trayDom.offset().left+trayDom.width());
		moveFaller.css({
			'width': ddl.moveFallerW
		});
	}
...
ddl.cut = function(){
	var moveFaller = $('.moveFaller'), moveFallerPrev =  moveFaller.prev() , trayDom = $('.tray');
	if(moveFaller.offset().left < moveFallerPrev.offset().left){
		ddl.moveFallerW = moveFaller.width() - (moveFallerPrev.offset().left - moveFaller.offset().left) ;
		moveFaller.css({
			'width': ddl.moveFallerW ,
			'left': moveFaller.prev().offset().left,
			'background-position': '-'+(moveFallerPrev.offset().left - moveFaller.offset().left)+'px 0'
		});
	}else if(moveFaller.offset().left > moveFallerPrev.offset().left ){
		ddl.moveFallerW = moveFaller.width() - ( moveFaller.offset().left  - moveFallerPrev.offset().left );
		moveFaller.css({
			'width': ddl.moveFallerW
		});
	}
...
//游戏开始
ddl.start = function(countFn,endFn,start){
	if((gameOver && gameOverCount == 0)|| (gameOver && start)){
 		gameOverCount++;
		gameOver = false; 
...
ddl.restart = function(){
	if(gameOver){
		...
		ddl.start(ddl.countFn,ddl.endFn,true);
