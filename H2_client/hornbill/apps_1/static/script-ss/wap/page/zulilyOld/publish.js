...
btnAdd.on("click", function() {
	...
	remain = max + 1 - len;
	var jsonParams = '{"action":"desire", "max":' + remain + ',"source":"photo","r":' + fml.vars.common_r + '}';
...
submit.on("click", function() {
	...
	$.ajax({
		type: 'POST',
		url: '/aj/zulilyOld/style_user_create',
		data: data,
		dataType: 'json',
		success: function(result) {
			...
			if (result.error_code) {
				...
				window.location.href = window.__get_r('/zulilyOld/personal?showTab=1' + inAppTabbarArg, fml.vars.common_r);
			}
		},
