 ...
 ********************/
fml.define('order_pc/common/validate', [], function(require, exports) {
	return {
		/*参数：
		----表单id;
		...
		*/
		validate: function(formName, validateRules, showStyle, options) {
			...
			//验证规则集合
			var ruleSet = {
				'req': validateRequired, //必填项			参数: req=表单元素的默认值
			...
			//验证结果样式集合
			var styleSet = {
				'showmsgbyline': showMsgByLine, //onblur逐行验证并返回结果
				'showmsgforsubmit': showMsgForSubmit, //提交表单时一次总验证并返回第一个错
				'showmsgbyone': showMsgByOne
			}; //一次总验证并在特定位置显示错误信息	showmsgbyone=obj1.obj2
			var msgTipBox = '';
