fml.define('order_pc/common/shopping_carts', ['jquery', 'component/iStorage', 'ui/alert', 'component/ajax' , 'core/animation'], function (require, exports) {
    var $ = require('jquery')
    ...
    function addAnimate(icoOrgin, from, to, cbk) {
        ...
        var fromOff = from.offset()
            , toOff = to.offset()
        var posStart = {
            'left': fromOff.left + from.width() * 0.4, 'top': fromOff.top
        }
        var posEnd = {
            'left': toOff.left, 'top': toOff.top + to.height()
        }
        ico.css(posStart)
    ...
    function upShoppintCartsNum(num, innerCacll) {
        var cartsBotNum = $('.bot_cart .cart_nums')
            , cartsTopNum = $('#myShoppingCartEl .num_bgc');
        if (num > 0) {
            cartsBotNum.text(num).show()
            cartsTopNum.show().removeClass('num_bgc_none').text(num)
        } else {
            cartsBotNum.hide()
            cartsTopNum.hide()
        }
    ...
    exports.readShopsCount = function () {
        iStorage.getCookie(goodsCountCacheKey, function (num) {
            ...
            ajax.aj('/aj/doota/cart_number', function (res) {
