...
fml.define('page/huodong/ms' , ['jquery' , 'ui/dialog' , 'component/shareTmp' , 'app/shareTo' , 'component/regString' , 'app/checkLogin'] , function(require,exports){
	$ = require('jquery');
	...
	var actName = 'ms_0';
	var ntime = new Date();
	if(ntime < new Date('2013/1/21')){
		actName = 'ms_0';	
	}else if(ntime >= new Date('2013/1/21') && ntime < new Date('2013/1/28')){
		actName = 'ms_1';	
	}else if(ntime >= new Date('2013/1/28') && ntime < new Date('2013/2/1')){
		actName = 'ms_2';	
	}
	var isAjaxLoad = true;
	var goodsInfo = {};
	...
	$('.ms_click').on('click' , function(){
		...
		$('.ms_sub3').unbind('click').click(function(){
			...
			var data = { actName : actName , pics : ms_pics , msPic_default : '/css/images/huodong/ms/ms_img5.jpg' };
			$.get('/wbapp/ajax_composepics' , data , function(res){
				$.get('/aj/getGoods/commercePost' , { tid : res , type : 'ms' } , function(data){
	...
	/*share sina qzone qq*/
	var url = location.href + '&frm=MS01';
	var desc = "我刚刚在美丽说参与了M&S马莎百搭大赛，各种风格任我搭，你也来玩吧！大奖高达1500元现金券，还有更多好礼等你来拿！@马莎中国 >>";
