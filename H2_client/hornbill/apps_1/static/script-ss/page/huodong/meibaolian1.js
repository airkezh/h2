fml.define('page/huodong/meibaolian1' , ['jquery' , 'app/checkLogin', 'app/tracking' , 'ui/dialog' , 'component/shareTmp' , 'app/shareTo'] , function(require , exports){
	var $ = require('jquery');
...
*/		
	...
	function postData(pid){
		var url="/aj/huodong/cus_create_twitter",
			data={
				pid:pid,
				actUrl:'/biz/huodong/bbrestaging1/?frm=meibaolianBB02',
				actTitle:'你是哪种BB肌',
	...
	function popResult(param){
		...
		$('#shareWB').on('click',function(){
			...
			_gaq.push(['_trackPageview', 'Share to Weibo']);
		});
		$('#buyNow').on('click',function(){
			logTrack.cr('meibaolian', {'frm': 'Buy now'});
			_gaq.push(['_trackPageview', 'Buy now']);
		});
	}
	...
	$('.radioGroup a').on('click',function(){
		...
		if(qn==2){
			if(index==0){
				...
				_gaq.push(['_trackPageview', 'Q11']);
			}
			else if(index==1){
				logTrack.cr('meibaolian', {'frm': 'Q12'});
				_gaq.push(['_trackPageview', 'Q12']);
			}
			else if(index==3){
				logTrack.cr('meibaolian', {'frm': 'Q13'});
				_gaq.push(['_trackPageview', 'Q13']);
			}
		}else if(qn==3){
			if(index==0){
				logTrack.cr('meibaolian', {'frm': 'Q21'});
				_gaq.push(['_trackPageview', 'Q21']);
			}
			else if(index==1){
				logTrack.cr('meibaolian', {'frm': 'Q22'});
				_gaq.push(['_trackPageview', 'Q22']);
			}
		}else if(qn==1){
			if(index==0){
				logTrack.cr('meibaolian', {'frm': 'Q31'});
				_gaq.push(['_trackPageview', 'Q31']);
			}
			else if(index==1){
				logTrack.cr('meibaolian', {'frm': 'Q32'});
				_gaq.push(['_trackPageview', 'Q32']);
			}
		}
	});
	...
	$('#detail').on('click',function(){
		...
		_gaq.push(['_trackPageview', 'DETAIL']);
	});
	...
	$('.bg1 .banner img').on('click',function(){
		...
		if(index==0){
			...
			_gaq.push(['_trackPageview', 'Surprise 1']);
		}
		else if(index==1){
			logTrack.cr('meibaolian', {'frm': 'Surprise 2'});
			_gaq.push(['_trackPageview', 'Surprise 2']);
		}
	});
	...
	$('.bg3 .track a').on('click',function(){
		...
		switch (cla){
			case 'c1':
				...
				_gaq.push(['_trackPageview', 'TMALL DRY']);
				break;
			case 'c2':
				logTrack.cr('meibaolian', {'frm': 'TMALL 18ml'});
				_gaq.push(['_trackPageview', 'TMALL 18ml']);
				break;
			case 'c3':
				logTrack.cr('meibaolian', {'frm': 'TMALL OIL'});
				_gaq.push(['_trackPageview', 'TMALL OIL']);
				break;
			default:
				break;
		}
