fml.define('m/app/doota/cart/goods',['m/zepto/touch', 'm/app/doota/shopping_carts','m/component/callApi'],function(require , exports){
	var shopping_carts = require('m/app/doota/shopping_carts')
	...
	var goods = {
		...
		_update: function(func){
			...
			callApi.write({'url':'/cart/update'}, { 'param': sid }, function(res){
				for (var i=0; i < res.info.length; i++) {
					goods.updateStock(res.info[i].sid,res.info[i].stock);
				};
				func();
			});
			goods.buffer = [];
		...
		del: function(ids,msg,successMsg){
			...
			var func = function(){
				...
				callApi.write({'url':'/cart/remove'}, {'sid':ids.join(',')}, function(res,status){
					if(res.code == 0){
						alert(successMsg)
