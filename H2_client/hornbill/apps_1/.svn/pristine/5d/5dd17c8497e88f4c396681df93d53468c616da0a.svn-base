fml.define('component/iStorage',[], function(require ,exports){
	...
	var IEStorage = {
		...
		//	html += '<param name="movie" value="' + fml.getOption('modulebase') + '../img/storage1.swf" />';  
		//	html += '<param name="movie" value="http://i.meilishuo.net/css/images/window/storage.swf?d='+date.getTime()+'" />';  
			html += '<param name="movie" value="http://i.meilishuo.net/css/images/window/storage1.swf" />';  
	...
	var CookieStorage = {
		set : function(key, value, session, fun) {
			session ? Cookie.setCookie(key, value) : Cookie.setCookie(key, value, {duration:6*30*24*3600});
			typeof fun == 'function' && fun();
		},
		get : function(key, session, fun) {
			fun(Cookie.getCookie(key));
		},
		remove : function(key, session) {
...
//	var storage = IEStorage.detectIE() ? IEStorage : DOMStorage;
	var storage = IEStorage.detectIE() ? function(){
		try {
	...
	return {
		...
		getAllowDomain : function() {
			var domains = domains || ["www.meilishuo.com", "newlab.meilishuo.com", "newtest.meilishuo.com", "wwwtest.meilishuo.com", "cdjdev.meilishuo.com", "xhdev.meilishuo.com", "rwdev.meilishuo.com", "nanodev.meilishuo.com"];
			return domains;
		},
		...
		getCookie : function(key ,cbk) {
			return Cookie.getCookie(key,cbk);
		},
		removeCookie : function(key) {
