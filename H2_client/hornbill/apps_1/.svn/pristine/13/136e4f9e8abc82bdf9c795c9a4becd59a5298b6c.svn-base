...
var curNavListLength = curPageData.cate_list.length || 0;
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if (code == 0) {
                var oLayer,
                    remindTpl,
                    remind = data.data.remind || 0;

                if (remind == 1) {
                    remindTpl = shareTmp('j_open_remind_tpl');

                    $('body').append(remindTpl);

                    oLayer = $('#j_open_remind_layer');

                    oLayer.animate({'opacity': 1}, 500, function(){
                        if (remindLayerTimeout) {
                            clearTimeout(remindLayerTimeout);
                            remindLayerTimeout = null;
                        }

                        remindLayerTimeout = window.setTimeout(function(){
                    ...
                    remindTpl = shareTmp('j_close_remind_tpl');

                    $('body').append(remindTpl);

                    oLayer = $('#j_close_remind_layer');
...
var main = {
    ...
    initEvent: function(){
        ...
        $('body').on('click', '.tracking', function(){
            ...
            tracking.cr('page_element', {'_action': 'click', 'r': '_page_id=1100001:_page_area=banner:_pos_id=' + pos + ':_pos_name=' + name});
        ...
        $('body').on('click', '.gallery_box li', function(){
            ...
            // 发送数据统计请求
            tracking.cr('page_element', {'_action': 'click', 'r': '_page_id=1100001:_page_area=event:_pos_id=' + pos + ':_pos_name=' + name});
            ...
            if (href == '') {
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        if (code == 0) {
                            ...
                            if (link != '') {
                                // 发送数据统计请求
                                //tracking.cr('member_center', {'area': 'top_wap', 'name': title, 'to_url': link});
                ...
                // 发送数据统计请求
                //tracking.cr('member_center', {'area': 'top_wap', 'name': title, 'to_url': href});
