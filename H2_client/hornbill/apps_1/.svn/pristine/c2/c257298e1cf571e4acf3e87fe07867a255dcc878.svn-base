...
supportBox.click(function() {
	...
	var personalURL = 'http://mapp.meilishuo.com/zulily/personal/?frm=list_bottom_left_heart' + inAppTabbarStr + stat_red_point;
	var url = Meilishuo.config.os.mlsApp ? openAppLink.getAppLink({
		'protocol': 'openURL',
		'param': {
			"url": personalURL
		}
	}) : "/zulily/personal/?frm=list_bottom_left_heart" + inAppTabbarStr + stat_red_point;
	window.location.href = url;
	// if(dot.hasClass("hide")){
	// 	window.location.href = "/zulily/personal/?frm=list_bottom_right_heart"+inAppTabbarStr;
	// }else if(dot.hasClass("show")){
	// 	window.location.href = "/zulily/personal/?frm=list_bottom_left_heart&&showTab=1"+inAppTabbarStr;
	// }else return;
});
/*通用Ajax函数*/
...
function getIngUrl(ele) {
	var style_id = parseInt(ele.attr('styleId')),
		...
		detailURL = 'http://mapp.meilishuo.com/zulily/detail/?style_id=' + style_id + '&frm=list_ing&trace_id=' + fml.vars.trace_id,
