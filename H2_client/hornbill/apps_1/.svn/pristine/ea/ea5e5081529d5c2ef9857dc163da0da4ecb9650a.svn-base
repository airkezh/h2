...
var navfixed = function(){
	...
	function isFade() {
		...
		if(!isScroll && pageScrollTop > bannerOffset + goJiH){
			isScroll = true
			$('.fixed_wrap').show()
		}
		if(isScroll && pageScrollTop <= bannerOffset + goJiH){
...
function addPoster(poster_name, r){
	$.get('/aj/promote/activity_item_list_1111', {'tab': poster_name,'contentid': fml.vars.acid}, function(res){
		res.r = r;
...
$banner.hide()
addPoster('全部', fml.vars.r + '.' + $('.poster_tab div').eq(0).attr('data_name'))
...
var tab_change = function(mSelf){
	...
	logTrack.cr('beauty_sale1111', {'frm': fml.vars.acid + '_' + $(self).attr('data_name')});
