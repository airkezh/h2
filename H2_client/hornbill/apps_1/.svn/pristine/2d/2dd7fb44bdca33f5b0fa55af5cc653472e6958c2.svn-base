 ...
 * @param callback
 */
var SimulationScrollX = function(config, callback) {
    ...
    this.notScrollOut = config.notScrollOut && true;
    // window.scrollTo(0,2);
    this.move();
}
SimulationScrollX.prototype = {
    destory : function() {
    ...
    move : function(e) {
        ...
        var  target = this.target, //移动目标
            ...
            bMove = monitor.offsetWidth - target.offsetWidth,//滑块到底top值,如果为正数，说明target高度小于视口高度，则不能向上滚动
        ...
        function moveStart(e) {
            ...
            if (e.touches)
                e = e.touches[0];
            startX = e.clientX;
            startLeft = target.style.marginLeft || tMove;
            //惯性缓动
        ...
        function moveIn(e) {
            if (flg) {
                ...
                scroller && (scroller.style.marginLeft = -move / st + 'px');
                //惯性缓动
        ...
        function moveEnd(e) {
            ...
            //惯性缓动
            var contentTop = target.style.marginLeft.replace('px', '');
            var contentY = parseInt(contentTop);
            var nowTime = new Date().getTime();
            var v = (e.clientX - lastMoveStart) / (nowTime - lastMoveTime); //系数，手指抬起的越慢，惯性越不明显，当足够慢时，惯性效果即可忽略
            //最后一段时间手指划动速度
            ...
            (function(v, startTime, contentY) {
                ...
                function inertiaMove() {
                    ...
                    if (dir * nowV > 0) {
                        if (move > tMove) {
                            callbackfn('到顶了');
                            target.style.marginLeft = tMove + 'px';
                            scroller && (scroller.style.marginLeft = tMove + 'px');
                        } else if (move < bMove) {
                            callbackfn('到底了');
                            target.style.marginLeft = bMove + 'px';
                            scroller && (scroller.style.marginLeft = -bMove / st + 'px');
                        }
                        setTimeout(function() {
                            if (!stopInertiaMove)
                    ...
                    //end事件滚出效果控制
                    if (_this.notScrollOut) { //这里可以配置一个参数，是否开启滚出效果，true为关闭
                        //滚出后复位
...
exports.SimulationScrollX = SimulationScrollX;
