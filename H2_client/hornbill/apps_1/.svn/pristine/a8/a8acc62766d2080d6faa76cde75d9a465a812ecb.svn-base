fml.define('page/huodong/act11' , ['app/doota/timedown','jquery','ui/dialog','component/shareTmp',  'ui/alert','app/login','app/checkLogin','component/windowScroll','ui/encrypt'] , function(require, exports){
	var	$ = require('jquery')
    ...
    function couponVenue(){
        $.ajax({
            ...
            success:function(data){
                ...
                var imgUrl = "http://i.meilishuo.net/css/images/biz/salePrice/";
                ...
                $.each(coupons, function( k , couponDate ){
                    ...
                    $(".couponBox"+ i).find('a').attr('data-name', 'encore_11');
                    ...
                    switch (couponDate.can_use){
                        ...
                        case 1:
                            ...
                            if( nextTime ){ 
                                ...
                                if( nowH >= 21 && nowH <=24 ){
                                    $(".couponBox"+i).find("img").attr("src",imgUrl+"tomorrow_11.png");
                                }else{
                                    $(".couponBox"+i).find("img").attr("src",imgUrl+ t +"time_11.png");   
                                }
                            }else{
                                $(".couponBox"+i).find("img").attr("src",imgUrl+ "empty_11.png");
                            }
                        break;
                        case 2:
                            if( new Date().getTime() < new Date('2014-11-11').setHours(0) ){
                                $(".couponBox"+i).find("img").attr("src",imgUrl+"again_11.png");
                            }else{
                                $(".couponBox"+i).find("img").attr("src",imgUrl+"active_11.png");
                            }
                        break;
                    }
                })
    ...
    function coupon(){
        $('.get_coupon').on('click',function(event){
            ...
            function checkCoupon(){
                if( imgPic != 'http://i.meilishuo.net/css/images/biz/salePrice/ing_11.png'){return;}
                $.ajax({
    ...
    //倒计时
    function getClock(){
        var end_time = '2014/11/12 00:00:00', s_time = '2014/11/11 00:00:00',start_time,end_time,tips,countdown_text,countdown_time;
        cur_time = new Date( current_time );
        start_time = new Date(s_time);
        end_time = new Date(end_time);
        if( cur_time < start_time ){
            tips = "nostart";
            countdown_text = '距离11.11还有';
            countdown_time = start_time;
        }else if(cur_time >= start_time && cur_time <= end_time){
            tips = "ing";
            countdown_text = '距离11.11结束还有';
            countdown_time = end_time;
        }else{ 
            tips = "end";
            countdown_text = '活动已结束';
        }
        if(tips != "end"){
