...
var controlFns = {
	...
	'daily' : function(){
		...
		var php = {
			...
			, 'ad_ban' : '/weixin/Weixin_mall_fashion_perfect'
		}
		this.setMDefault(php);
		this.bridgeMuch(php);
		this.listenOver(function(data){
	...
	'ad_temp' : function(args){
		var pageName = this.req.__get.page || 'daily'
		var php = {
		...
		this.bridgeMuch(php);
		this.listenOver(function(data){
			data.cate_id = this.readData('cate_id',this.req.__get, 0);
			data.pageName = pageName;
			data.share = !!(data.tempInfo.share_img && true);
			data.item_name = 'clothing';  /* 三级菜单需求暂时不做 */
			data.pageTitle = data.tempInfo.title || '美丽说购时尚';
	...
	'search' : function(args){
		...
		this.listenOver(function(data){
			data.pageName = 'daily';
			data.key = this.req.__get.key || '';
			data.cate_id = this.readData('cate_id',this.req.__get, 0);
			data.item_name = 'clothing';  /* 三级菜单需求暂时不做 */
			data.pageTitle = '美丽说';
			data._CSSLinks = ['wx_new/ad_temp','wx_new/class'];
	...
	// 		}
	// 		data.pageTitle = '评论种草'
	// 		data._CSSLinks = ['wx/comment']
	// 		this.render('wx/comment.html', data)
	// 	})
	// },
    'aquarius': function (){
        'use strict'
