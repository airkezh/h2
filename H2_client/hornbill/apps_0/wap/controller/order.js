...
var controlFns = {
	...
	'init' : function(){
		...
		this.listenOver(function(data){
			...
			data._CSSLinks = ['doota/confirm_new'];
			data.pageTitle = '订单确认页 - 美丽说';
			mSelf.render('doota/confirm_new.html',data);
		});
	},
	'init_old' : function(){
	...
	'create' : function(){
		...
		this.listenOver(function(data){
			...
			// 订单页风控
			if(data.order.code == -102){
				return mSelf.redirectTo('/welcome/check_code/?rule_id=' + data.order.info.rule_id)
			}
		...
		//	console.log(data.order)
			if (data.order.form) {
				...
				// 0元购
				if(data.order.no_paid_purchase){
					return mSelf.redirectTo('/myCenter')
				}
				// 去收银台，合单
				if(data.order.total_id){
					return mSelf.redirectTo('/cashier?orderno=' + data.order.total_id + '&merchantcode=' + data.order.merchant_id + '&biztype=1&transtype=2');
				}
				// 去收银台，单单
				if(data.order.order_id){
					return mSelf.redirectTo('/cashier?orderno=' + data.order.order_id + '&merchantcode=' + data.order.merchant_id + '&biztype=1&transtype=2');
				}
