...
api.read({'url':'/cart/list_info'},{}, function(data){
	...
	if(tn > 2 && oneTime){
		// 猜你喜欢
		$.get('/aj/sq/interfaces/goods_recommend', function (res){
			if(res.error_code === 200){
				var tpl = shareTmp('sliders_tpl', res)

				$('#guess_u_like').append(tpl).show()
				oneTime = false
			}
		}, 'json')
	}
...
$cart.on('click', '.del,.del-all', function(){
	...
	new Confirm({
			content : '是否确认删除已选商品?'
			, onSure : function(){
				api.write({'url': '/cart/remove'}, {'sid':_this.attr('data-sid')}, function(data){
					if(data.code == 0){
						location.reload()
					}
				});
...
$cart.on('click', '.go_buy', function(){
	...
	if(total_price() <= 0){
		...
		if(go_buy_confirm){
			...
			location.href = '/sq/orderconfirm?sid=' + sidStr + '&shop_id=' + shop_id + '&wechat_card_js=1'; 
		}
	}
})
...
var EventProcessor = {
	openPanel : function (){
		...
		var $t = this
		var url = '/coupon/coupon_shop'
		var data = {
			shop_id: $t.data('shop_id')
		}

		isOpenPanel = true
		api.read({url: url}, data, function (res){
			$couponList.append(shareTmp('coupon_tpl', res))
			$couponPanel.show()
		})
	},
	...
	applyCoupon : function (){
		if(this.hasClass('got') || isInApply) return

		var $t = this
		var url = '/coupon/coupon_apply'
		var data = {
			coupon_apply_code: $t.data('code')
		}

		isInApply = true

		api.write({url:url}, data, function (res){
			isInApply = false

			if(res.code === 0){
				$t.toggleClass('get got')
				myAlert('领取成功，可到个人中心查看')
			}else{
				myAlert(res.message)
			}
		})
...
$('body').on('click', function (e){
	var $target = $(e.target)
	var fn = $target.data('action')
	var _e = EventProcessor[fn]

	_e && _e.call($target)
})

$couponPanel.on('touchmove', function (e){
	e.preventDefault()
})

function myAlert(ctn, cbk){
	return new Alert({
			content: ctn,
			onSure : cbk
		})
