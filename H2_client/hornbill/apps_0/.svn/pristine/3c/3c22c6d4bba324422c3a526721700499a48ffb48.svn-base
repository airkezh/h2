...
var controlFns = {
	...
	'simple' : function(){
		...
		var php = {
			...
			, 'risk_tip' : 'im::/im/open_get_riskinfo'
		}
		var mSelf = this
	...
	'www' : function(){
		...
		this.listenOver(function(data){
			...
			//console.log('recently', data.recently.data.datas);
	...
	'higo_shoper' : function(){
		...
		var php = {
			// 'recently' : 'im::/im/recently?source=web&to=' + toid
			'recently' : 'im::/im/open_higo_recently?source=web&limit=50&channel_id=2&app=higirl'
		...
		this.listenOver(function(data){
			...
			if(data.userInfo.user_id == 0)
				//return mSelf.redirectTo('/www/login/' ,true);
				return mSelf.redirectTo('http://b.higo.meilishuo.com' ,true);
	...
	//higo 客服
	'higo_server' : function(){
		var toid = this.req.__get.toid | 0

		var php = {
			// 'recently' : 'im::/im/recently?source=web&to=' + toid
			'recently' : 'im::/im/open_higo_recently?source=web&limit=50&channel_id=2'
			, 'udr' : 'im::/im/udr_get?channel_id=2'
			, 'transfer' : 'im::/im/transfer_list?channel_id=2'
			//, 'historyUser' : 'im::/im/detaillist?channel_id=2'
			, 'publicUser' : 'im::/im/public_detaillist?channel_id=2'
		...
		this.listenOver(function(data){
			...
			if(data.userInfo.user_id == 0)
			//return mSelf.redirectTo('/www/login/' ,true);
				return mSelf.redirectTo('http://hgadmin.higo.meilishuo.com' ,true);
...
//		this.debugSnake({target : 'devlab3'});
	...
	aj : function(params){
		...
		var php = {
			...
			, 'get_service' : 'im::/im/open_get_service'
			, 'open_higo_online_ticket_create' : 'im::/im/open_higo_online_ticket_create'
			, 'ticket_create' : 'im::/im/Open_higo_ticket_create'
			, 'welcome' : 'im::/im/open_greeting'
			, 'im_shop' : 'im::/im/im_shop'
		}
		this.ajaxTo(php[params])
	}
