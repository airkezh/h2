...
var controlFns = {
	'special':function(event_id){
		...
		var wapMod = base.loadModel('wap/tools.js'),
			shareURL = 'http://m.meilishuo.com/activity/tuan/special/'+event_id;
		...
		this.listenOver(function(data){
			...
			/*share*/
			var params = {
				'title' : data.special_list.data.share_info.share_title || '',
				'text' : data.special_list.data.share_info.share_text || '抢8【1元秒杀】每晚8点，限量优品整点抢购。美丽说团购年度巨献！',
				'pic' : data.special_list.data.share_info.share_image || data.PICTURE_URL + "images/wap/activity/tuan/n_qiang8.jpg",
				'url' : shareURL
			};
	...
	'hd' : function(event_id){
		...
		var curtab = this.req.__get.curtab;
		var wapMod = base.loadModel('wap/tools.js'),
		...
		var php = {
	        'actinfo' : 'groupon::/miaosha/Miaosha_Top_Banner?event_id='+event_id
	    };
		this.bindDefault(php);
		this.bridgeMuch(php);
		this.listenOver(function(data){					
	...
	'life' : function(){
		var php = {
			'goodsList' : 'groupon::/groupon/Groupon_hot_tuan_list'
			, 'banner' : 'groupon::/groupon/groupon_get_banners_mob?locationKey=mob_tuan_top'
		};
		var mSelf = this;
		this.bindDefault(php);
		this.bridgeMuch(php);
		this.listenOver(function (data){
			if(!data.goodsList) return mSelf.errorPage();
			data.use_rem_screen = true;
			data.pageTitle = '爆款专区';
			data._CSSLinks = ['tuan/activity/life'];
			mSelf.render('activity/tuan/life.html', data);
		})
	},
	'highQuality' : function(){
		var php = {
			'banner' : 'groupon::/groupon/groupon_get_banners_mob?locationKey=mob_rucang_top_banner'
			, 'rucang' : 'groupon::/groupon/groupon_rucang_goods_list?event_id=2666&limit=100'
		};
		var ua = this.req.headers['user-agent'];
		var mSelf = this;
		var platform = false;
		...
		this.bridgeMuch(php);
		this.listenOver(function (data){
			if(!data.rucang) return mSelf.errorPage();
			data.host = (this.req.headers && this.req.headers.host) || 'm.meilishuo.com';
			data.isIphone = platform;
			data.use_rem_screen = true;
			data.pageTitle = '品质优选';
			data._CSSLinks = ['tuan/activity/highQuality'];
			mSelf.render('activity/tuan/highQuality.html', data);
		})
	},
	'qualityControl' : function(){
		...
		this.listenOver(function (data){
			...
			data.pageTitle = '官方质检';
			data._CSSLinks = ['tuan/activity/highQuality'];
			mSelf.render('activity/tuan/qualityControl.html', data);
		})
	}
}
