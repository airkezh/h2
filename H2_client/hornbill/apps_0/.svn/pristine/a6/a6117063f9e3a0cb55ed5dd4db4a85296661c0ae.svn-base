...
var controlFns = {
    ...
    wallet_act : function(actid) {
        ...
        this.listenOver(function(data){
            data.isSetMobile = data.userInfo.mobile_is_actived;
            data.isSupportVersion = false;
            var os = wapMod.uaos(mSelf.req);
            if (os.android) {
                data.isSupportVersion = wapMod.isNewest(mSelf.req, "5.3.0");
            } else if (os.ios) {
                data.isSupportVersion = wapMod.isNewest(mSelf.req, "5.2.1");
            }
