...
$(".apply_btn").on( "click" , function(){
	...
	$(".form_li input").focus( function(){
		if( this.value == '' || this.value == '请填写11位手机号码' || this.value == '链接请以http://开头' || this.value == '10位数字推id，多个推id请用“，”间隔'){
			this.value = '';
		}
	});
	...
	$(".submit_btn").on( "click" ,function(){
		...
		check_form( checkName , checkTel , checkQq , checkShop , checkId );
		
	})
})
function check_form( checkName , checkTel , checkQq , checkShop, checkId ){
	...
	if( checkShop == '' || checkShop == '链接请以http://开头'){
		$(".check_shop").next("span").text("链接错误！");
		return;
	}
	// var urlReg = /[a-zA-z]+:\/\/[^\s]*/;
	...
	// }
	var idReg = /^(\d{10}(,|，)?)+$/;
	if( idReg.test(checkId) == 0){
		$(".check_id").next("span").text("推id错误");
		return;
	}
	$.ajax({
