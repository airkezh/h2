fml.define('app/posterWalls' , ['component/userstate' , 'component/iStorage', 'component/waterfall' ,'app/referrer' , 'app/checkLogin' , 'component/windowScroll' , 'jquery' , 'component/shareTmp' ] , function(require , exports){
	...
	var getPoster = function(urlData , posterUrl , options){
		...
		var callback = function(response){
			...
			if(response){
				...
				if(((Math.floor(goodsPage) == urlData.page) && (response.tInfo && response.tInfo.length < frameSize)) || (response.tInfo && response.tInfo.length == 0) || !response.tInfo){
					showPg();
				}
			   ...
			   fml.vars.toLogPoster && window.setTimeout(function(){
				...
				if(urlData.frame >= 10 && !(options && options.Infinite)){
					...
					*/
					...
					if(response.tInfo && +response.totalNum > +frameSize && Math.ceil(response.totalNum/frameSize) > urlData.frame){
						isPosterLoad = true;
						urlData.frame++;
						fml.vars.frame = urlData.frame;
					}else{
	...
	return function(urlData , posterUrl , options){
		...
		if (poster0 && poster0.tInfo && poster0.tInfo.length > 0){
			minHeight = pin.getMinCH();
			urlData.frame++
			isPosterLoad = true;
			//统计单品坑位用
