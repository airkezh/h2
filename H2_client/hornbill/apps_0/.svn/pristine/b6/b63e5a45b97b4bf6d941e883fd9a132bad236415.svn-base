 ...
 * @param {source} string 页面来源，不是必须(会员项目)
 * @param {isOpen} string 代表是否默认打开APP，默认打开，如果值为true，则不打开APP（不管是否安装），而是停留在原页面
 */
function Open(url, daownUrl, text, isPop, source, isOpen) {
...
return function(url, daownUrl, text, isPop, source, isOpen) {
    return new Open(url, daownUrl, text, isPop, source, isOpen);
}