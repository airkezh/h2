fml.define('page/common/shopping_carts' , ['jquery', 'component/iStorage','ui/alert','component/ajax' ,'core/animation'] , function(require , exports){
    var $  = require('jquery')
    ...
    function upShoppintCartsNum(num , innerCacll){
        var cartsBotNum = $('.bot_cart .cart_nums')
            ,cartsTopNum = $('.mycart .num_bgc')
        if (num > 0){
            cartsBotNum.text(num).show()
            cartsTopNum.removeClass('num_bgc_none').text(num)
        }else{
            cartsBotNum.hide()
            cartsTopNum.addClass('num_bgc_none');
            }
