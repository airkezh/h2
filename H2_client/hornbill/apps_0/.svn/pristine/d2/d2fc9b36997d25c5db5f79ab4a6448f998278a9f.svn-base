...
var beans_coupon_promotion = {
    ...
    initEvent: function(){
        ...
        $('body').on('click', '.go_other_btn', function(){
    ...
    sendCouponAjax: function(that){
        ...
        var name = (flag == 'plat') ? 'double11_coupon' : 'shop_coupon';
    ...
    updateCouponState: function(oThat, data){
        ...
        if (flag == 'plat') {
            oThat.find('.lave_text, .beans_text, .convert_text').css('color', '#ccc');
        }
    ...
    renderCouponHtml: function(type){
        ...
        var name = (type == 'plat') ? 'double11_coupon' : 'shop_coupon';
                    ...
                    if (jsonData) {
                        if (type == 'plat') {
                            var couponData = jsonData.self_coupon || [],
                                tpl = shareTmp('plat_coupon_tpl', {'couponData': couponData, 'hours': fml.vars.hours, 'couponTitle': fml.vars.platCouponTitle});

                            $('#plat_coupon_box').html(tpl);
                        } else if (type == 'shop') {
                            var couponData = jsonData.shop_coupon || [],
                                tpl = shareTmp('shop_coupon_tpl', {'couponData': couponData, 'hours': fml.vars.hours, 'couponTitle': fml.vars.shopCouponTitle, 'ruleTitle': fml.vars.activityRuleTitle, 'ruleData': fml.vars.couponRuleTextArr});
