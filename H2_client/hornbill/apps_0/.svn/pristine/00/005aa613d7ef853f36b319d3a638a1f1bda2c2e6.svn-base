...
var topDialog = function(toid, type, notIntoView){
	...
	if(type == 'new'){
		if(toid!=fml.vars.im.toid){
			$box.find('.num').html(userListObj[toid].num);
			$box.addClass('newMsg')
...
var addDialog = function(data){
	...
	if(getUserCount() == 1 && userListObj[uid]){
		userListObj[uid].num = 0;
		changeUser(uid);
		unreadTipObj.fresh();
	}
...
function getUserCount(){
	var n, count = 0;
	for(n in userListObj){
		if(userListObj.hasOwnProperty(n)){
			count++;
		}
	}
	return count;
...
exports.unreadNumFresh = unreadNumFresh
