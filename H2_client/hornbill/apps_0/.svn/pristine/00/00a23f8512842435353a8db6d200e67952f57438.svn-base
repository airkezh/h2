fml.define('wap/app/login' , ['wap/zepto/touch', 'component/storage', 'component/urlHandle', 'component/validate','component/shareTmp','app/checkcode'] , function(require , exports){
	...
	var checkLogin = function(user,pass,checkcode,state){
		var url = '/aw/user/logon'; 
		var data = {
			'login_name' : $(user).val(),
			'password' : $(pass).val(),
			'checkcode' : checkcode ? $(checkcode).val() : '',
		...
		//		orgUrl = "/settings/bindMobile?orgUrl=" + orgUrl;
			...
			switch(r.status){
				...
				//	if(Meilishuo.config.controller === 'login'){//页面登陆
					/* @hack 找回密码页登陆成功后跳首页 */
					if(orgUrl.indexOf('/user/findPwd') >= 0){
						urlHandle.redirect(window.__get_r('/welcome'), fml.vars.global_r);
					}else{
						urlHandle.redirect(window.__get_r(orgUrl), fml.vars.global_r);
					}
				//	}else{//弹窗登陆
				...
				case -4 :
					...
					var t = setTimeout(function(){
						urlHandle.redirect(window.__get_r(r.redirect), fml.vars.global_r);
					},5000);
					break;
				case -21:
					window.location.reload();
	...
	var showLogin = function(){
		...
		formValidate(function(){
			checkLogin('#mlsUser','#mlsPass','#checkcode','#savestate');
		});
	};
	var showLoginWin = function(){
		urlHandle.redirect(window.__get_r('/user/login'), fml.vars.global_r);
	};
