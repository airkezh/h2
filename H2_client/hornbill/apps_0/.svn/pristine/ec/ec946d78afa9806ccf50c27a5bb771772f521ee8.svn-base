...
var changePrice = function(coupon_type,addr_res){
	...
	if(coupon_type == 'address' && addr_res){
		...
		data.street = addr_res.addressDetailInfo
...
$('body').on('click','#charge', function(){
	...
	data.goods_pid = channel_mark ? (data.goods_pid + '-' + channel_mark) : data.goods_pid;
