...
var controlFns = {
	...
	'goods' : function(attrName){
		...
		var php = {
			...
			'notic' : '/goods/catalog_keywords?nid=210175'
		};
...
//				data.isShowGuangBanner = true;
	...
	'catalog' : function(catalogid){
		...
		var pricebt = this.readData('fl_rg_price', this.req.__get);
		...
		var mSelf = this;
		var php = {
			'catalog' : '/goods/Catalog_keywords?nid=' + nid,
			'catalog_nav' : '/navigate/navigate_words',
			'poster0' : '/goods/catalog_poster' + hrefsearch,
			'share_pic' : '/goods/Share_outside_mix',
			'rec_poster' : '/navigate/Navigation_recommend'
			, 'season' : '/customactivity/CustomActivity_seasonal_special_banner'
			,'catTitle' : '/goods/catalog_info'
			,'search_types' : '/search/search_type'
			,'sizer' : '/search/search_filter?nid=' + nid + (tag_word ? '&tag_word=' + tag_word : '') 
		};
		...
		*/
		...
		this.listenOver(function(data){
			...
			if(cata_id == '2001000000000'){
				catalogid = 'jacket';
			}else if(cata_id == '2004000000000'){
				catalogid = 'skirt';
			}else if(cata_id == '2006000000000'){
				catalogid = 'pants';
			}else if(cata_id == '2009000000000'){
				catalogid = 'underwear';
			}
			...
			data.groupPg.total_num = data.poster0.usableNum;
			data.showTotalNum = data.poster0.showNum;
			data.groupPg.current_page = page;
			data.attrName = catalogid; 
			...
			if (nid){
				mSelf.render('guang/catalogThree.html' , data);
			}else{
