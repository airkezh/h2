...
var lazyLoad = lazyLoad.init({xpath:'.more_pic>span'})
lazyLoad.load()
lazyLoad.scroll()
...
$('.recommend').on('tap', function() {
	...
	fallAdd({
		'tid': fml.vars.goods.twitter_id,
		'cbk': rollCommentPnl,
		'url': '/aj/getComment/shop?filter=1&filter_point=0',// 取4/5星的评论
		'processData': function(res) {
			return {
				'item': res.cInfos,
				'totalNum': res.pages.totalNum
...
$('#cmt_tab').on('tap','.tab_list',function(){
	...
	fallAdd({
		'tid': fml.vars.goods.twitter_id,
		'cbk': rollCommentPnl,
		'url': '/aj/getComment/shop?filter=1&filter_point='+cur,// 取4/5星的评论
		'processData': function(res) {
			return {
				'item': res.cInfos,
				'totalNum': res.pages.totalNum
