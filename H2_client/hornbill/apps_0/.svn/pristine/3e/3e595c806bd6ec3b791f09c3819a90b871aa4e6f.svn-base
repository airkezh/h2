...
var $ = require('jquery')
    ...
    , sendAjax = require('ui/sendAjaxPic')
...
function bind(inp , opt){
    ...
    //var wrapper = wrapInp(inp ,opt).on('change' , 'input[type=file]' ,sendHigoPic );
    console.log('uploadBtn bind');

    function sendHigoPic(o){
        console.log('sendHigoPic', o);
        Api({
            url: 'http://v.higo.meilishuo.com/image/upload',
            params: {
                token: co.im_id
            },
            success: function(resp){
                console.log('sendHigoPic success', resp);
            }
        });
    }

    function sendFile(o){
        console.log('uploadBtn sndFile ',o);
        if(opt.FilesAdded){
        ...
        var ifr = $('<iframe id="'+ifrId+'" name="'+ifrId+'"><script id="'+ifrId+'_"></script></iframe>')
            .hide()
            .appendTo('body')
        var _timer
        if (opt.timeout) {
        ...
        if(opt.crossDomain){
            if(opt.success){
                ...
                window[_callbackName] = function(ret){
                    console.log('uploadBtn bind ret',ret);
                    try {
                        ret = $.parseJSON(ret)
                        opt.success(ret , inp)
                    }catch(err){
                ...
                if(co && co.im_id){
                    crossDomainData += ('&token=' + co.im_id);
                    crossDomainData += ("&callback="+_callbackName);
                }
        ...
        var form = $('<form action="'+ opt.behind+crossDomainData +'" target="'+ifrId+'_" method="post" enctype="multipart/form-data">'+plusData+'</form>')
