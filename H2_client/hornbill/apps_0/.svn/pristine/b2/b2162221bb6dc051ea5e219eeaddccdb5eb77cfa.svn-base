...
;(function(window,document){
    ...
    $.fn = $.prototype = {
        ...
        closest : function(classSel){
            //与init sel匹配重叠，先提供class用着
            classSel = classSel.slice(1);
            var closestParent = null;
            var node = this[0];
            while(node && node.nodeType === 1){
                if(node.className.indexOf(classSel) > -1){
                    closestParent = node;
                    break;
                }else{
...
;(function($){
    ...
    $.changeNav = function(opt){
        conf = opt;
        $('#def-nav').hide();
        $('#com-nav').append($(conf.el)[0]);
        $('#nav').css('height', $('#com-nav')[0].scrollHeight + 'px');
    }

    $.restoreNav = function(){
        if(conf && conf.restoreContainer){
            $(conf.restoreContainer).append($(conf.el)[0]);
        }
        $('#def-nav').show();
        $('#nav').css('height', $('#com-nav')[0].scrollHeight + 'px');
    }
...
;(function($){
    ...
    // 各种环境配置
    var httpLabs = [
        'newlab.meilishuo.com',
        'qalab.meilishuo.com',
        'rdlab.meilishuo.com',
        'fedevot.meilishuo.com'
    ];
    for (var i = 0; i < httpLabs.length; i++) {
        if(host.lastIndexOf(httpLabs[i]) > -1){
             domain = 'http://' + host + '/';
             break;
        }
    };
    ...
    function bindSeach(){
        ...
        // 还原search状态
        var sk = /(?:&|\?)searchKey=([^&]+)/.exec(location.search);
        if(sk && sk[1]){
            searchInput.val(decodeURI(sk[1]));
        }
        var filter = /(?:&|\?)filter=([^&]+)/.exec(location.search);
        filter = filter && filter[1];
        if(filter == 'shop'){
            changeType.call($('span',tab)[1]);
        }
        ...
        function selectItem(dir){
            ...
            cur.addClass('active');
            searchInput.val(cur.attr('data-title'));
        }

        function jump(searchKey){
            if(selIndex == 0){
                searchKey = $.trim(searchKey) || $.trim(searchInput.attr('data-key'));
            }else{
            ...
            if(selIndex != 0){
                ...
                pstrc = '&pstrc=' + encodeURIComponent('searchshop=' + searchKey);
            }
        ...
        suggBox.on('mouseover',function(e) {
            ...
            var title = $('.active', this).attr('data-title');
            var target = e.target;

            if(target.tagName.toLowerCase() == 'span'){
                jump(title + ' ' + $(target).text());
            }else{
                jump(title);
            }
        });
	...
	$.init = function(opt){
        ...
        var upConf = ['topbar','search','nav','foot'];
        $.each(upConf,function(index,value){
            var oriVal = conf[value];
            if(typeof oriVal == 'string'){
                conf[value] = {
                    container : oriVal
                }
            }
