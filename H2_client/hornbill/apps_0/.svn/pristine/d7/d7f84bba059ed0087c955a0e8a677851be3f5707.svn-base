...
var controlFns   = {
    ...
    'bindMobile': function( args ) {
        ...
        this.listenOver( function( data ) {
            //（0已绑定；1审核中；2审核被拒绝）
            if ( data.userInfo.wait_audit == 1 || data.userInfo.wait_audit == 2 ) {
                this.redirectTo( '/user/audit/new' )
            } else {
                data.mlsApp = mlsApp;
                if ( args == 'change' ) {
    ...
    'newMobile':  function( args ) {
        var php   = {};
        var mSelf = this;
        this.setMDefault( php );
        this.bridgeMuch( php );
        this.listenOver( function( data ) {
            data.pageTitle      = '验证新手机号'
            data.use_rem_screen = '640';
            data._CSSLinks      = [ 'newMobile' ];
            mSelf.render( 'user/newMobile.html', data );
        } )
    },
    'apply':      function( args ) {
        // this.debugSnake({target : 'xiaolongrong.rdlab'});
        var php   = {
            'selectYear':   '/user/Change_bind_time_picker'
            , 'addrSelect': '/user/Addr_select'
        };
        var mSelf = this;

        if ( args == 'edit' ) {
            var editFlag        = true;
            php[ 'historyMsg' ] = '/user/Save_audit_info?type=get'
        }

        var changeMobile = mSelf.readData( 'change_mobile', this.req.__get, '' );

        this.setMDefault( php );
        this.bridgeMuch( php );
        this.listenOver( function( data ) {
            data.changeMobile   = changeMobile;
            data.editFlag       = editFlag;
            data.pageTitle      = '安全验证'
            data.use_rem_screen = '640';
            data._CSSLinks      = [ 'apply' ];
            mSelf.render( 'user/apply.html', data );
        } )
    },
    'audit':      function() {
        var php   = {};
        ...
        this.listenOver( function( data ) {
            data.pageTitle      = '审核中'
            data.use_rem_screen = '640';
            data._CSSLinks      = [ 'audit' ];
            mSelf.render( 'user/audit.html', data );
        } )
    },
    'check_code': function( args ) {
        var php = {
            ...
            punish_type = this.readData('type', this.req.__get, '');
