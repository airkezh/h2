...
var $           = require( 'wap/zepto' ),
    ...
    DEFAULT_DANMU_TEXT = '我喜欢',
    USER_REG_VALIDITY_TIME = 7 * 24 * 60 * 60 * 1000,
    ...
    WX_SHARE_PIC         = 'http://i.meilishuo.net/css/images/activity/danmu/newcomer/wx_share_pic.jpg',
    WX_SHARE_TEXT        = '美丽说新人奖，邀请新人抓红包，100块钱给你了！',
    WX_SHARE_TEXT_SUC    = ' $ 元优惠券到手！终于等到你，还好我没放弃╭(╯3╰)╮',
    WX_SHARE_TEXT_FAIL   = '含泪握拳！红包红包我一定要抓住你！',
    WX_SHARE_URL         = 'http://' + LOCATION.host + '/wx/newcomer/';
...
Bullet.prototype = {
    ...
    initAttr: function( content, type ) {
        var speedRatio, style, width, top, left, dist,
        ...
        this.bonusWidth + 80 || ( this.bonusWidth = el.offsetWidth + 80 ) : signaleEl.offsetWidth + 80
        ...
        top = this.channel * ctx.channelHeight + topOffset

        if ((type == BONUS) && (top < 30)) {
                top += 70
        }

        this.stylePrefix = 'left:' + left + 'px;top:' + top + 'px;'

        style.cssText = this.stylePrefix + vendorTransform.join( left )
...
function Panel( el ) {
    ...
    this.closeBtn = DOC.getElementById( 'close-panel-btn' )
...
Panel.prototype = {
    ...
    show: function( type, id ) {
        ...
        if (type == 'timeout') {
            ...
            closeBtn.innerHTML      = config.share
            closeBtn.setAttribute( 'data-type', 'timeout' )
            $face.addClass(face)
            this.title.innerHTML    = config.title
            this.desc.innerHTML     = desc[ 0 ]

            this.$el.show()
            this.$el[ 0 ].className = 'panel-fail'
        } else {
            okBtn.style.display     = 'inline-block'
            shareBtn.style.display  = 'inline-block'
            closeBtn.style.display  = 'none'
            shareBtn.innerHTML      = config.share
            okBtn.setAttribute( 'data-type', type )
            shareBtn.setAttribute( 'data-type', type )

            if ( id ) {
                title = '啦啦啦 中' + lotteryData.awards[ id ].awardname + '噜～'
...
;(function() {
    ...
    EventHandlers = {
        ...
        bonus: function( el ) {
            ...
            Tracking.cr( 'redpaper_click', {'action': 'click'} )
        ...
        hidePanel: function( el ) {
            ...
            var name = (type == 'timeout') ? 'humility' : 'undertone'
            Tracking.cr( 'redpaper_click', {'action': 'click', 'status': type, 'name': name} )
        },
        ...
        sharePanel: function( el ) {
            panel.hide()
            var type = el.data( 'type' )
            var name = (type == 'fail') ? 'spit' : 'flaunt'
            Tracking.cr( 'redpaper_click', {'action': 'click', 'status': type, 'name': name} )
        ...
        shoot: function() {
            ...
            var msg = DEFAULT_DANMU_TEXT + (msg || $input.val())
        ...
        fastShoot: function( el ) {
            ...
            var msg = DEFAULT_DANMU_TEXT + el.text();
