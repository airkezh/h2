...
var icon         = 'http://d02.res.meilishuo.net/pic/_o/fd/fc/78f4fb3bc73dc592b1483475ac5f_90_80.ch.png'
			...
			dataCashe[index].waterfallInstance = Waterfall.init({
				...
				dataFilter: function (data){
					...
					if(params.frame === 0){ 
						...
						if(gDatalength == 0){
							$currentContent.empty().append(RenderTpl('js-empty-page-tpl', {prompt: '您还没有收藏的商品哦', img: icon}))
							return false
						}
					}
				...
				dataFilter: function (data){
					...
					if(frame === 0){
						...
						if(data[gd].length == 0 && index == 0){
							$currentContent.append(RenderTpl('js-empty-page-tpl', {prompt: '您还没有收藏的店铺哦', img: icon}))
							return false
						}
					}
