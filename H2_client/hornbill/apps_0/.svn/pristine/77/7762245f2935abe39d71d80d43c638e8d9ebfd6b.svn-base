 ...
 * 未完成
 */
fml.define('wap/component/swipeScale', [], function(require, exports){
	return function (container, options) {
		...
		function scale(index, delay_x, act, speed){
			var slide = slides[index].children[0]
			var style = slide && slide.style;

			if (!style) return;

			if(act == 0){
				var scale_num = (((426 - Math.abs(delay_x)) / 426) * 0.2 + 0.8)
			} else {
				var scale_num = 1 - ((426 - Math.abs(delay_x)) / 426) * 0.2
			}
			scale_num = scale_num > 1 ? 1 : scale_num
			...
			style.webkitTransform = 'scale(' + scale_num + ')';
		}
			...
			move : function(event){
				...
				if (!isScrolling) {
					...
					if (options.continuous) { // 如果是一个没有端点无限循环的幻灯片则不增加滑动阻力
						...
						if(delta.x < 0){
							scale(circle(index + 1), delta.x, 1, 0)
						} else {
							scale(circle(index - 1), delta.x, -1, 0)
						}
						scale(index, delta.x, slidePos[index], 0, 0)
				...
				if (!isScrolling) {
					if (isValidSlide && !isPastBounds) {
						...
						scale(index, 0, 0, speed)
						scale(circle(index + 1), width, 0, speed)
						scale(circle(index - 1), width, 0, speed)
