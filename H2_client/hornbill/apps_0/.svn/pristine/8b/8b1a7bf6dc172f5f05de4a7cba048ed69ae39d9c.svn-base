	...
	, goods = {
		...
		updateBuffer: function(data){
			...
			for(var i=0; i < goods.buffer.length; i++){
				if(goods.buffer[i].id === id){
					return goods.buffer[i].num = data.num
				}
			}
		...
		add: function(input, msg, max){
			var val = parseInt(input.val())
				, max = isNaN(Number(max)) ? Infinity : max;	// 判断max是不是数字，如果不是，默认无上限
		...
		minus: function(input, msg, min){
			var val = parseInt(input.val())
				, min = isNaN(Number(min)) ? -Infinity : min;	// 怕段min是不是数字，如果不是，默认无下限
