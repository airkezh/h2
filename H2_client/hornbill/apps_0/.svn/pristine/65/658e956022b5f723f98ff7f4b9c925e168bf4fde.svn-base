...
var controlFns = {
	index : function(tid){
		...
		var php = {
			...
			, 'attr_rec_new' : '/goods/share_attr_lite_new'
	...
	'item' : function(tid){
		var wapMod = base.loadModel('wap/tools.js'),
			os = wapMod.uaos(this.req),
			mSelf = this,
			page = this.readData('page', this.req.__get, 0),
			frm = this.readData('frm', this.req.__get, ''),
		...
		var php  = {
			...
			, 'format' : '/share/goods_specifications?__get_r=1'
			, 'circle_banner' : '/circle/item_circle_banners?__get_r=1'
			, 'goods_comments' : '/goods/comment_list?wxmall=1&filter=1&__get_r=1' // 取4/5星的评论
			, 'single_promotion' : '/coupon/item_single_promotion?twitter_id=' + tid + '&__get_r=1'
			, 'hot_goods' : '/shop/wap_shop_goods?shop_id=115547&limit=18&__get_r=1'
			, 'seo_words' : '/item/Item_recommend_word'
			, 'goods_popular' : '/item/Item_recommend_for_all?__get_r=1'
		...
		this.listenOver(function(data){
			...
			data._serviceqq = data.goods_info.data.qq;
			data._shopid = data.goods_info.data.shop_id;
			data.goods_info.data.is_doota = '1';
			data.isSale = true;
			data.SaleChannel = true;
			data.isTopcart = true;
			data.isM = true;
			...
			//普通单品
				...
				data._CSSLinks = ['share/item_new'];
				mSelf.render('share/item_new.html', data);
			}
		});
	},
	'item_detail' : function(tid){
		var mSelf = this,
			frm = this.readData('frm', this.req.__get, ''),
			tab = this.readData('tab', this.req.__get, 0);
		...
		this.listenOver(function(data){
			...
			data._CSSLinks = ['share/item_detail'];
			mSelf.render('share/item_detail.html', data);
		});
	}
}
