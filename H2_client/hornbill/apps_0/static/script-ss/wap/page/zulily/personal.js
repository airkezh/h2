...
function appendFun(obj) {
	...
	// if (!isPrevTab) {
	obj.append('<p class="btn-index"><a href="/zulily/?showTab=1' + inAppTabbarArg + '" xr="' + fml.vars.common_r + '">返回主页</a></p>');
	// }
}
...
window.MLS = {
	onUploadComplete: function(json) {
		...
		if (obj) {
			...
			window.location.href = window.__get_r('/zulily/publish' + inAppTabbarArg, publishXR);
		} else {
			alert('数据解析失败，请重试！');
		}
	}
