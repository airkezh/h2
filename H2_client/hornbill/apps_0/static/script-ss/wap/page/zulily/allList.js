...
if (!hasSticky()) {
	scrollHeight = scrollHeight - tab.height() * 2 - searchBoxShow.height() * 2;
} else {
	scrollHeight = scrollHeight - tab.height() - searchBoxShow.height();
}
	...
	$(window).scroll(function() {
		...
		if (t > scrollHeight) {
			sortBox.css({
				'position': 'fixed',
				'left': 0,
				'top': tab.height() + searchBoxShow.height() + 'px'
			});
		} else {
			sortBox.css({
				'position': 'static'
