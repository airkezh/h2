...
if (fml.vars.twitter_id) {
	getWall(2, '/aj/desire/index/detailWaterfall?&limit=' + limit + '&twitter_id=' + fml.vars.twitter_id + '&__get_r=1', 'posterWall', false);
}
...
if ($('.danmu').length) {
	...
	var danmu = Danmu({
		...
		initBullet: function() {
			var childrenDiv = $('.danmu').children('div');
			if (childrenDiv.text() != '') {
				childrenDiv.addClass('danmu-block');
			}
	...
	$send.on('click', function() {
		...
		var commentVal = $comment.val().replace(/^\s*|\s*$/g, '');
		var data = {
