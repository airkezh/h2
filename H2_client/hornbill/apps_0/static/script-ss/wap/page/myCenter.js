...
var downloadApp = function(frm) {
	$('.noDownload').on('click', shotDwon)

	$('.downloadBtn').on('click', function(){
		window.open('/goto/open/?bg=' + encodeURIComponent('http://m.meilishuo.com/download/latest/?channel=30902&frm=' + frm) + '&url=' + encodeURIComponent('meilishuo://open.meilishuo'))
	})
}
function shotDwon(){
...
var download_show = function(frm){
...
var myLove = function() {
	...
	$.post('/aj/doota/order/second_menu', data, function(res){
		$('#myLove').html(res.likeNum)
		$('#myShop').html(res.shopNum)
	},'json');
}
...
;(function(){
	...
	$('.tab_content').on('click', '.js_download_btn', function(){
		download_show($(this).data('frm'))
	})
