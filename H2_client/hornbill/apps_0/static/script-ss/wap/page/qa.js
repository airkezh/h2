...
shareWX.bind({
	'link': 'http://'+location.host + '/qa/',
    'desc':"",
    'imgUrl':"http://d05.res.meilishuo.net/img/_o/11/f4/74bd515be202b840ff8b5d2eaff0_300_300.cf.jpg",
    'title':"都不是LOW人，都敢上擂台！"
});
...
function showCurResult(data) {
	...
	*/
	 ...
	 if(cur_score == 0){
	 	$(".level0").show();
	 }else if(cur_score >= 1 && cur_score <= 10) {
	 	$(".level1").show();
	 }else if(cur_score >= 11 && cur_score <= 30) {
	 	$(".level2").show();
	 	$(".result .surpass").show();
	 } else if(cur_score >= 31 && cur_score <= 40) {
	 	$(".level3").show();
	 	$(".result .surpass").show();
	 } else {
	 	$(".level4").show();
...
//向后台发送本局分数和时间,并获得历史最好成绩,发送成功后显示此成绩
function sendData(data){
		$.ajax({
			url:'/aj/qa/set',
			data:{
				correct_num: data.score
				,answer_time: data.cost_time
				,open_id: open_id
			},
			type:'post', 
			dataType:'json',
