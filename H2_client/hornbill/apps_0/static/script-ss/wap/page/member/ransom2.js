...
var posterWalls = {
    ...
    loadPoster: function(urlData, data){
        ...
        if (isFirstAjaxLoad) {
            isFirstAjaxLoad = false;

            $('#page_container').html(shareTmp('page_tpl', data));
        }
...
var ransom = {
    ...
    initEvent: function(){
        ...
        $('body').on('click', '.today_recom_wrap li', function(){
            var that = $(this),
                pos = that.data('pos') || 0,
                tid = that.data('tid') || 0,
                href = that.data('href') || '',
                r = fml.vars.r + ':_page_area=up:_pos_id=' + pos;


            //if (href.indexOf('r=') < 0) {
                //href += ((href.indexOf('?') >= 0) ? '&' : '?') + 'r=' + r;
            //}
        ...
        $('body').on('click', '.goods_wall li', function(){
            if (fml.vars.serverHours < fml.vars.activityHours) {
                ...
                var that = $(this),
                    ...
                    tid = that.data('tid') || 0,
                    href = that.data('href') || '',
                    r = fml.vars.r + ':_page_area=down:_pos_id=' + pos;
