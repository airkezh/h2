...
if($('.nav_tabs').length){
	$navLi.width($nav_tabs.width()/$navLi.length);
...
if(Meilishuo.config.os.mobileQQ){
	$('body').on('click', '.productBox a',function(e){
		e.preventDefault();
...
//侧边栏数据
if(fml.vars.isShow && !Meilishuo.config.os.mobileQQ){
	$.post("/aj/huodong/navi_data", {"actid" : 419}, function(data) {
