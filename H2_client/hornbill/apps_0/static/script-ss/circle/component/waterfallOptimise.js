...
$win.on( FORCE_CHECK, function() {
    if ( globalBoxes && globalBoxes.length ) {
        globalBoxes._curTop = -9999
        optimise( globalBoxes, $win.scrollTop() )
    }
} )

/*
 列数不同，显示的元素总数也不同，1 列瀑布流会最多显示 7 个元素
 */
exports.initRange = function( col ) {
    OFFSET = rangeBase * col
    RANGE  = 2 * OFFSET + 1
}
