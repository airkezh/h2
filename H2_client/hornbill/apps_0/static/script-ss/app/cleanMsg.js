fml.define('app/cleanMsg' , ['jquery','page/im/open' , 'core/animation' , 'component/iStorage' /*'component/audio','component/userstate'*/ , 'component/animate'] , function(require , exports){
	...
	*/
	...
	function renderMsg(clear){
	 ...
	 $("#global-sidebar").on('click', '.openIM'  , function(){
			imOpen.reCallImWin({is_simple: true})
	...
	function imTip(customer_num){
		var siteImNm = $( '#side-im span' )
		if ( siteImNm.length ) {
			if(customer_num > 0) {
				if ( customer_num > 99 ) {
					customer_num = '99+'
					siteImNm.addClass( 'gt99' )
				} else {
					siteImNm.removeClass( 'gt99' )
				}
				siteImNm.text(customer_num).show()
			} else {
				siteImNm.hide()
			}
		}
