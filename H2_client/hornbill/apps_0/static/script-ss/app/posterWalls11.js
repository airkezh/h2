fml.define('app/posterWalls11' , ['component/userstate' , 'component/iStorage', 'component/waterfall' ,'app/referrer' , 'app/checkLogin' , 'component/windowScroll' , 'jquery' , 'component/shareTmp' ] , function(require , exports){
	var $ = require('jquery');
	...
	var getPoster = function(urlData , posterUrl , options){
		...
		var callback = function(response){
			if (response.code === 0) response = response.data;
			...
			if(response){
			   ...
			   fml.vars.toLogPoster && window.setTimeout(function(){
				...
				//大于5帧 停止继续加载	
				//Infinite为1时为无限下拉加载
				if(urlData.frame >= 7 && !(options && options.Infinite)){
					showPg();
		...
		$.get(posterUrl , urlData , callback , 'json').fail(function(){
			$('.spinner').hide()
		})
	}
