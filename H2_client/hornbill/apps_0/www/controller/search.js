...
var controlFns = {
	'index' : function(){
		...
		if(filter == 'goods'){
			...
			return this.redirectTo('/search/searchShop/', true);
		}
		try{
...
//			console.log(data.p4p)
	...
	'baobei' : function(){
		...
		php = {
			...
			,'goods_classify' : '/search/search_recommend?word_name=' + encodeURIComponent(searchKey)
			,'poster0' : '/goods/search_poster' + hrefsearch
			,'sizer' : '/search/search_filter?tag_word=' + encodeURIComponent(searchKey)
		};
		if (searchKey != '')
			php['p4p'] = '/pfp/pfp_hot?key_id=' + encodeURIComponent(searchKey) + '&tp=4'
		...
		this.listenOver(function(data){
			...
			data.groupPg = {}; 
			data.groupPg.page_size = 220;
			data.groupPg.total_num = data.poster0.goods.usableNum;
			data.showTotalNum = data.poster0.goods.showNum;
	...
	'getPosters' : function(args){
		...
		var result = {
			...
			recommend = {
				...
				'goods' : '/search/Search_without_result_recommend',
	...
	'searchShop': function(){
		...
		var php = {
			'search_shop_totalnum' : '/shop/search_totalnum',
			'shop_list': '/shop/search_shop_new?word_name=' + encodeURIComponent(searchKey)
		}

		this.setMDefault(php);
		this.bridgeMuch(php);
		this.listenOver(function(data){
			if(!data['search_shop_totalnum'].totalNum){
				return this.redirectTo('/search/emptyShop/', true);
			}
			data.sWord = searchKey;
			data._CSSLinks = ['searchShop'];
			data.pageTitle = (searchKey == '' ? '' : searchKey + ' - ') + '店铺' + '搜索 － 美丽说';
			this.render('search/searchShop.html', data);
		});
	},
	'emptyShop' : function(){
		var searchKey = this.readData('searchKey',this.req.__get, '').trim();
		...
		var php = {
			'shop_list': '/shop/search_recommend_new'
		}
		var mSelf = this;
		this.setDefaultData(php);
		this.bridgeMuch(php);
		this.listenOver(function(data){
			data.sWord = searchKey;
			data._CSSLinks = ['shopEmpty'];
			data.pageTitle = (searchKey == '' ? '' : searchKey + ' - ') + '店铺搜索 － 美丽说';
			mSelf.render('search/shopEmpty.html' , data);
		});
