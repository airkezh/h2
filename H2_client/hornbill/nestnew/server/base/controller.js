...
function checkdIllegal (hostPath){
	...
	if ('www/' != hostPath) return true
	if ('Zabbix-MLS (Meilishuo monitor system)' ==  this.req.headers['user-agent'])	return true

	var refer = this.req.headers.referer
		,reqUrl = this.req.url
	if (/^\/a(j|w|jax)\//i.test(reqUrl) && (!refer || -1 == refer.indexOf('meilishuo.com') || !this.isTokenLive(30) )) {
