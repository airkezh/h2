...
exports.spinDry = function(data ,php){
	var oKeys =  Object.keys(php || data)
  
    oKeys.forEach(function(key,index){
        var item = data[key]
		if (!item) return
        if ('object' != typeof item || !('code' in item) || !('data' in item)) return 
        if (item.code) return data[key] = false
        else return data[key] = item.data
        })
}
...
exports.loadModel = function(modName){
	var modPieces = modName.split('/')		
	var shortMap = {
        'm' : 'm',
		'wap' : 'wap',
		'mls' : 'www'
		}
	var host = shortMap[modPieces[0]]
	if (host) {
		host += '/'
		modName = modPieces.slice(1).join('/')
	}
	return require (config.path.appPath + host +  config.path.model + modName)
}
/*
*继承并创建新对象*/
