...
function route(request ,response ) {
    ...
    var virtualHostname = config.virtualHost[reqUrl.hostname],
    	hostNametype = (typeof virtualHostname).toLowerCase(),
    	urlTest;
    if(hostNametype == 'string' || hostNametype == 'undefined'){
    	virtualHostname = virtualHostname || reqUrl.hostname;
    }else if(hostNametype == 'object'){
    	for(var item in virtualHostname){
    		if(virtualHostname[item] != ''){
    			urlTest = new RegExp(virtualHostname[item],['g']);

    			if(urlTest && urlTest.test(reqUrl.pathname)){
    				virtualHostname = item;
    				break
    			}
    		}else{
    			virtualHostname = item;
	...
	**/
	...
	if (ua && 'www/' == hostPath && (ua.indexOf('iphone')>0 || ua.indexOf('android')>0 ) ){
